...
module.exports = class ChessCommand extends BaseCommand {
  constructor() {
    super('chess', 'activity', []);
  }
  ...
  run(client, message, args) {
    const channel = message.mentions.channels.first() || message.guild.channels.cache.get(args[0]);
        ...
        fetch(`https://discord.com/api/v8/channels/${channel.id}/invites`, {
            ...
            .then(res => res.json())
            .then(invite => {
                if (invite.error || !invite.code) return message.channel.send("❌ | Could not start **Chess In The Park**!");
                let chessembed = new Discord.MessageEmbed()
                .setTitle('Chess In The Park')
                .setColor('FF0000')
                .setThumbnail(client.user.displayAvatarURL())
                .setDescription(`✅ | Click here to start **Chess In The Park** in <#${channel.id}>:\n <https://discord.gg/${invite.code}>`)
                .setFooter(`executed by: ${message.author.username}`, message.author.displayAvatarURL());
                message.channel.send(chessembed);
            })
            .catch(e => {
                message.channel.send("❌ | Could not start **Chess In The Park**!");
            })
  }
}