...
module.exports = class FishingtonCommand extends BaseCommand {
  constructor() {
    super('fishington', 'activity', []);
  }
  ...
  run(client, message, args) {
    const channel = message.mentions.channels.first() || message.guild.channels.cache.get(args[0]);
        ...
        fetch(`https://discord.com/api/v8/channels/${channel.id}/invites`, {
            ...
            .then(res => res.json())
            .then(invite => {
                if (invite.error || !invite.code) return message.channel.send("❌ | Could not start **Fishington.io**!");
                let fishingtonembed = new Discord.MessageEmbed()
                .setTitle('Fishington.io')
                .setColor('FF0000')
                .setThumbnail(client.user.displayAvatarURL())
                .setDescription(`✅ | Click here to start **Fishington.io** in <#${channel.id}>:\n <https://discord.gg/${invite.code}>`)
                .setFooter(`executed by: ${message.author.username}`, message.author.displayAvatarURL());
                message.channel.send(fishingtonembed);
            })
            .catch(e => {
