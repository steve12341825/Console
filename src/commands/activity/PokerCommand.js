  ...
  run(client, message, args) {
    const channel = message.mentions.channels.first() || message.guild.channels.cache.get(args[0]);
        ...
        fetch(`https://discord.com/api/v8/channels/${channel.id}/invites`, {
            ...
            .then(res => res.json())
            .then(invite => {
                if (invite.error || !invite.code) return message.channel.send("❌ | Could not start **Poker Night**!");
                const pokerembed = new Discord.MessageEmbed()
                .setTitle('Poker Night')
                .setColor('FF0000')
                .setThumbnail(client.user.displayAvatarURL())
                .setDescription(`✅ | Click here to start **Poker Night** in <#${channel.id}>:\n <https://discord.gg/${invite.code}>`)
